...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  // RBAC Flags
  const showOverview = role === 'ADMIN' || role === 'DOCTOR' || role === 'RESEARCHER';
  const showQueue = role === 'DOCTOR' || role === 'NURSE';
  const showClinicalActions = role === 'DOCTOR';
  const showRecords = role === 'DOCTOR' || role === 'NURSE' || role === 'RESEARCHER';
  const showKnowledge = role === 'ADMIN' || role === 'DOCTOR' || role === 'RESEARCHER';
  ...
  return (
    <aside className={`sidebar ${isOpen ? 'mobile-open' : ''}`}>
      ...
      <div className="sb-section">
        ...
        {showQueue && (
          <Link href="/queue" className={`sb-item ${pathname.startsWith('/queue') || pathname.startsWith('/discharge') || pathname.startsWith('/radiology') ? 'active' : ''}`}>
