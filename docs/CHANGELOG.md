...
## [2026-05-05] — Module 8: Đào tạo AI và Model Management
### Added
- **UC-MOD-01**: Tích hợp Model Card hiển thị thông số hiệu năng và an toàn của mô hình (Metrics: PHI Leak, Hallucination).
- **UC-MOD-02**: Cơ chế Active Learning (Mockup). Hệ thống ghi nhận các chỉnh sửa của Bác sĩ thông qua hàm `approveReport` và chuẩn bị dữ liệu cho quá trình fine-tuning.
- **UC-MOD-03**: Giao diện Trung tâm Đào tạo (Model Registry). Theo dõi phiên bản mô hình, ngày cập nhật và quy mô tập dữ liệu huấn luyện.
### Files changed
- `src/app/training/page.tsx` (NEW)
- `src/app/page.tsx` (MODIFIED)
- `src/services/api.ts` (MODIFIED - thêm `approveReport`)
