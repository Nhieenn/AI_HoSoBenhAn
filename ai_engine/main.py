...
# Cấu hình CORS để cho phép Frontend từ tên miền quany.test9.io.vn gọi API
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://quany.test9.io.vn", "https://quany.test9.io.vn", "*"], 
...
generator = ClinicalGenerator(rag_engine=rag_engine)
...
class ActiveLearningDiffRequest(BaseModel):
    original_text: str
    edited_text: str

class ActiveLearningSaveRequest(BaseModel):
    instruction: str
    input_text: str
    output_text: str
...
@app.post("/training/active-learning/diff")
async def training_al_diff(request: ActiveLearningDiffRequest):
    try:
        diff_summary = active_learning_mgr.extract_diff(request.original_text, request.edited_text)
        return {"diff": diff_summary}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/training/active-learning/save")
async def training_al_save(request: ActiveLearningSaveRequest):
    try:
        result = active_learning_mgr.save_to_dataset(request.instruction, request.input_text, request.output_text)
        return result
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/training/finetune/start")
async def training_finetune_start(request: FineTuningStartRequest):
    try:
        lora_config = fine_tuning_pipeline.configure_lora()
        result = fine_tuning_pipeline.run_sft_finetuning(request.dataset_path, lora_config, request.model_id)
        return result
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
